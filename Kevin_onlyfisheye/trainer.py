	...
	def train_one_epoch(self, epoch):
		...
		with tqdm(total=self.num_train) as pbar:
			for i, (x, y) in enumerate(self.train_loader):
				...
				baselines = []
				# log_probas_list = []
				for t in range(self.num_glimpses-1): #- 1):
				...
				adjusted_reward = R - baselines.detach() ## (R-b_freeze)
				loss_reinforce = torch.sum(-log_pi*adjusted_reward, dim=1) ## likelihood * (R- b_freeze) 
