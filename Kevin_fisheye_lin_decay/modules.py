...
class retina(object):
	...
	def __init__(self, g, k, s):
		self.g = g ## 8 size of high res patch
		self.k = k ## 1 num of patches
		self.s = s ## 2 scale between successive patches
	...
	def foveate(self, x, l):
		...
		# phi = torch.cat(phi, 1)
		# print("phi0 shape= ",phi[0].shape)
		# print("phi1 shape = ", phi[1].shape)
		phi[1][:,:,1:-1,1:-1] = phi[0]
		phi = phi[1]
...
class glimpse_network(nn.Module):
	...
	def __init__(self, h_g, h_l, g, k, s, c):
		...
		self.retina = retina(g, k, s) ## 8, 1, 2

		# glimpse layer
		# D_in = k*g*g*c ## 1 * 8*8 *1 (channels)  = 64
		D_in = 100
		self.fc1 = nn.Linear(D_in, h_g) ## 64 in x 128 out

		# location layer
		D_in = 2
		self.fc2 = nn.Linear(D_in, h_l) ## 2 in x 128 out

		self.fc3 = nn.Linear(h_g, h_g+h_l) # 128 x (128+128)
		self.fc4 = nn.Linear(h_l, h_g+h_l) # 128x (128+128)
...
class baseline_network(nn.Module):
	...
	def forward(self, h_t):
		...
		return b_t  ## b_t is Expectation of rewards at time t = value function
